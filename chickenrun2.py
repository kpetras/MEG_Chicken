...
from HelperFuns import monitor_selections
...
def run_experiment(participant_number, experience_level, session_number, feedback=True, group='experimental', n_trials=100):
    ...
    for count, trial_file in enumerate(file_paths, start=1):
        ...
        def on_pick(event):
            ...
            if event.artist:
                ch_index = event.ind[0]
                ch_name = trial_data.ch_names[ch_index]
                if ch_name in shared['selected_channels']:
                    shared['selected_channels'].remove(ch_name)
                    print(f'Removed {ch_name} from selection.')
                else:
                    shared['selected_channels'].add(ch_name)
                    print(f'Added {ch_name} to selection.')
                print(f'Current selected channels: {shared["selected_channels"]}')
...
def on_submit():
    ...
    run_experiment(participant_number, experience_level, session_number)
